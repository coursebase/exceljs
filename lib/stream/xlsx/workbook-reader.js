...
class WorkbookReader extends EventEmitter {
  ...
  async *_parseSharedStrings(entry) {
    ...
    /* https://github.com/exceljs/exceljs/issues/2063 */
    /* https://github.com/exceljs/exceljs/issues/2084 */
    entry.setEncoding('utf-8');

    let insidePhoneticReading = false;
    let text = null;
    let richText = [];
    let index = 0;
    let font = null;
    for await (const events of parseSax(entry)) {
      for (const {eventType, value} of events) {
        if (eventType === 'opentag' && !insidePhoneticReading) {
          const node = value;
          switch (node.name) {
            case 'b':
              font = font || {};
            ...
            case 'vertAlign':
              ...
              break;
            case 'rPh':
              insidePhoneticReading = true;
              break;
          }
        } else if (eventType === 'text' && !insidePhoneticReading) {
